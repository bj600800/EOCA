...
//			}
	...
	private static void update(ArrayList<String> positive, ArrayList<String> negative) {
		...
		for (int i = 0; i < positive.size(); i++) {
			...
			for (int j = 0; j < matrix_size; j++) {
				to_n1i[j] += (temp_c1i[j]*alpha);
				to_n1j[j] += (temp_v1j[j]*alpha);
				to_n2i[j] += (temp_c2i[j]*alpha);
				to_n2j[j] += (temp_v2j[j]*alpha);
		...
		for (int i = 0; i < positive.size(); i+=2) {
			Double C_1i = Double.parseDouble(positive.get(i).split(";")[0].split(",")[0]);
			Double V_1i = Double.parseDouble(positive.get(i).split(";")[0].split(",")[1]);
			
			Double C_1j = Double.parseDouble(negative.get(i).split(";")[0].split(",")[0]);
			Double V_1j = Double.parseDouble(negative.get(i).split(";")[0].split(",")[1]);
			
			Double C_2i = Double.parseDouble(positive.get(i+1).split(";")[0].split(",")[0]);
			Double V_2i = Double.parseDouble(negative.get(i+1).split(";")[0].split(",")[1]);
			
			Double C_2j = Double.parseDouble(negative.get(i+1).split(";")[0].split(",")[0]);
			Double V_2j = Double.parseDouble(negative.get(i+1).split(";")[0].split(",")[1]);
		
			// 正样本的频率和负样本的频率
			Double Pn1 = Math.pow(Double.parseDouble(positive.get(i).split(";")[1])*Double.parseDouble(positive.get(i+1).split(";")[1]), 0.75);
			Double Pn2 = Math.pow(Double.parseDouble(negative.get(i).split(";")[1])*Double.parseDouble(negative.get(i+1).split(";")[1]), 0.75);
			
			//组成4个向量
			Double[] n1i = new Double[matrix_size*2];
			Double[] n1j = new Double[matrix_size*2];
			Double[] n2i = new Double[matrix_size*2];
			Double[] n2j = new Double[matrix_size*2];
			for (int j = 0; j < matrix_size; j++) {
				n1i[j] = matrix.get(C_1i)[j];
				n1i[j+matrix_size] = matrix.get(V_1i)[j];
				
				n1j[j] = matrix.get(C_1j)[j];
				n1j[j+matrix_size] = matrix.get(V_1j)[j];
				
				n2i[j] = matrix.get(C_2i)[j];
				n2i[j+matrix_size] = matrix.get(V_2i)[j];
				
				n2j[j] = matrix.get(C_2j)[j];
				n2j[j+matrix_size] = matrix.get(V_2j)[j];
			}
	
			// //存储4个向量的梯度
			Double[] temp_n1i = new Double[n1i.length];
			Double[] temp_n1j = new Double[n1j.length];
			Double[] temp_n2i = new Double[n2i.length];
			Double[] temp_n2j = new Double[n2j.length];
	
			for (int j = 0; j < n1i.length; j++) {
				temp_n1i[j] = 0.0;
				temp_n1j[j] = 0.0;
				temp_n2i[j] = 0.0;
				temp_n2j[j] = 0.0;
			...
			// 计算梯度
			Double theta_n1 = 0.0;
			if ((Pn1 + Pij(n1i, n1j)) != 0) {
				theta_n1 = Pn1 / (Pn1 + Pij(n1i, n1j));
			}
			Double theta_n2 = 0.0;
			if ((Pn2 + Pij(n2i, n2j)) != 0) {
				theta_n2 = -Pij(n2i, n2j) / (Pn2 + Pij(n2i, n2j));
			}
			for (int j = 0; j < matrix_size*2; j++) {
				temp_n1i[j] = n1j[j] * theta_n1;
				temp_n1j[j] = n1i[j] * theta_n1;
				temp_n2i[j] = n2j[j] * theta_n2;
				temp_n2j[j] = n2i[j] * theta_n2;
			}
			...
			gt_c += temp_c*(1-alpha);
			// 对向量的梯度进行累加
			Double[] to_C_1i = new Double[matrix_size];
			Double[] to_C_1j = new Double[matrix_size];
			Double[] to_C_2i = new Double[matrix_size];
			Double[] to_C_2j = new Double[matrix_size];
			
			Double[] to_V_n1i = new Double[matrix_size];
			Double[] to_V_n1j = new Double[matrix_size];
			Double[] to_V_n2i = new Double[matrix_size];
			Double[] to_V_n2j = new Double[matrix_size];
			
			for (int j = 0; j < matrix_size; j++) {
				to_C_1i[j] = All_gt.get(C_1i)[j];
				to_C_1j[j] = All_gt.get(C_1j)[j];
				to_C_2i[j] = All_gt.get(C_2i)[j];
				to_C_2j[j] = All_gt.get(C_2j)[j];
				
				to_V_n1i[j] = All_gt.get(V_1i)[j];
				to_V_n1j[j] = All_gt.get(V_1j)[j];
				to_V_n2i[j] = All_gt.get(V_2i)[j];
				to_V_n2j[j] = All_gt.get(V_2j)[j];
			}
			for (int j = 0; j < matrix_size; j++) {
				to_C_1i[j] += temp_n1i[j]*(1-alpha);
				to_C_2i[j] += temp_n2i[j]*(1-alpha);
				to_C_1j[j] += temp_n1j[j]*(1-alpha);
				to_C_2j[j] += temp_n2j[j]*(1-alpha);
				to_V_n1i[j] += temp_n1i[j+matrix_size]*(1-alpha);
				to_V_n1j[j] += temp_n1j[j+matrix_size]*(1-alpha);
				to_V_n2i[j] += temp_n2i[j+matrix_size]*(1-alpha);
				to_V_n2j[j] += temp_n2j[j+matrix_size]*(1-alpha);
			}
			All_gt.put(C_1i, to_C_1i);
			All_gt.put(C_1j, to_C_1j);
			All_gt.put(C_2i, to_C_2i);
			All_gt.put(C_2j, to_C_2j);
			All_gt.put(V_1i, to_V_n1i);
			All_gt.put(V_1j, to_V_n1j);
			All_gt.put(V_2i, to_V_n2i);
			All_gt.put(V_2j, to_V_n2j);
		}
		for (int i = 0; i < alp.size(); i++) {
